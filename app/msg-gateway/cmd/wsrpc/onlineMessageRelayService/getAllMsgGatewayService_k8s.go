...
type k8sSvc struct {
	...
	lock              sync.Mutex
	port              int
	endpointZrpcMap   map[string]zrpc.Client
}
...
func (s *k8sSvc) Update() {
	...
	s.endpoints = nil
	s.endpointZrpcMap = make(map[string]zrpc.Client)
...
func parseTarget(target string) (*k8sSvc, error) {
	...
	return &k8sSvc{
		...
		port:              port,
		endpoints:         nil,
		endpointZrpcMap:   make(map[string]zrpc.Client),
	}, nil
}
...
func GetAllByK8s(
	...
	for _, endpoint := range ks.endpoints {
		if client, ok := ks.endpointZrpcMap[endpoint]; ok {
			...
			if e != nil {
				...
				services = append(services, NewOnlineMessageRelayService(client))
				ks.endpointZrpcMap[endpoint] = client
			}
		}
	}
	return
